...
function toAdmin(report: Awaited<ReturnType<TriageStore["getReport"]>>) { return report && ({ id: report.reporte_id, folio: report.reporte_id, description: report.texto, status: citizenStatus(report), area: report.area_responsable, category: report.categoria, priority: report.prioridad, assigneeId: report.asignado_a, createdAt: report.created_at, location: { address: report.location?.address, lat: report.coordenadas[0], lng: report.coordenadas[1] }, patternDetected: Boolean(report.pattern_detected), similarCount: report.similares_encontrados ?? 0, patternNote: report.patron_nota, acknowledgmentSent: Boolean(report.acuse_enviado), agentStatuses: report.agent_statuses ?? {}, agentConclusions: report.agent_conclusions ?? {} }); }
