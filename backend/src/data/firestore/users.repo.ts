...
export interface Usuario { id: string; nombre: string; rol: RolUsuario; telefono?: string; }
...
export function createUsersRepository(db: Firestore, auth: Auth): UsersRepository {
  ...
  return {
    ...
    async asignarRol(id, rol) { await auth.setCustomUserClaims(id, { rol }); await collection.doc(id).set({ rol }, { merge: true }); },
  };
}
