...
export class FirestoreTriageStore implements TriageStore {
  ...
  async listReports() { const snap = await this.db.collection("reportes").orderBy("created_at", "desc").get(); return snap.docs.map((doc) => doc.data() as Report); }
  async updateReport(id: string, update: Partial<Pick<Report, "area_responsable" | "asignado_a" | "progreso" | "prioridad" | "notas">>) { const ref = this.db.collection("reportes").doc(id); const current = await ref.get(); if (!current.exists) return undefined; await ref.set(update, { merge: true }); return { ...(current.data() as Report), ...update }; }
  ...
  async completeJob(job: IngestionJob, report: Report, ticket?: Ticket) { const batch = this.db.batch().set(this.db.collection("jobs_ingesta").doc(job.job_id), { estado: report.estado === "revision_manual" ? "revision_manual" : "completado" }, { merge: true }).set(this.db.collection("reportes").doc(report.reporte_id), { ...report, ticket_id: ticket?.ticket_id }, { merge: true }); if (ticket) batch.set(this.db.collection("tickets").doc(ticket.ticket_id), ticket, { merge: true }); await batch.commit(); }
