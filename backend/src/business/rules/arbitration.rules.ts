...
export interface ArbitrationInput {
  ...
  urgencia: Urgency;
  similares: number;
  causaEstructural: boolean;
  cercaEscuela: boolean;
  modifier: boolean;
}

/**
 * Applies arbitration rules to determine final urgency and priority.
 *
 * Rules:
 * - Rule 1: >=6 similar reports + structural cause → at least "alta"
 * - Rule 2: risk category near school → "critica"
 * - Modifier: raises urgency by one level (if not already "critica")
 */
