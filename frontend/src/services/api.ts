...
function toReport(record: { id: string; folio: string; status: string; createdAt: string; location: LocationValue; description?: string; area?: string; category?: string; priority?: Priority; assigneeId?: string; patternDetected?: boolean; similarCount?: number; patternNote?: string; acknowledgmentSent?: boolean; agentStatuses?: Partial<Record<"classifier" | "pattern" | "acuse", "done" | "error">>; agentConclusions?: Partial<Record<"classifier" | "pattern" | "acuse", string>> }): ReportRecord {
  const operationalStatus: Record<string, ReportRecord["operationalStatus"]> = { received: "new", processing: "review", ready: "new", failed: "review", assigned: "assigned", in_progress: "in_progress", resolved: "resolved" };
  ...
  const fallbackConclusions = { classifier: ticket ? `Clasificó el reporte como ${ticket.category} y lo canalizó a ${ticket.area}.` : "Pendiente de clasificación.", pattern: record.patternNote ?? `${record.similarCount ?? 0} coincidencias similares encontradas por el RAG.`, acuse: record.acknowledgmentSent ? "El acuse fue enviado al ciudadano." : "El acuse está pendiente de envío." };
  const agentConclusions = (["classifier", "pattern", "acuse"] as const).map((agent) => ({ agent, text: record.agentConclusions?.[agent] || fallbackConclusions[agent] }));
  return { id: record.id, folio: record.folio, citizenName: "", phone: "", description: record.description || "Reporte ciudadano", location: record.location, status: record.status === "resolved" ? "ready" : record.status === "assigned" || record.status === "in_progress" ? "processing" : record.status === "failed" ? "failed" : "received", operationalStatus: operationalStatus[record.status] || "review", channel: "form", createdAt: record.createdAt, updatedAt: record.createdAt, assignee: record.assigneeId, team: record.area, agents: { classifier: { agent: "classifier", status: status("classifier") }, pattern: { agent: "pattern", status: status("pattern") }, acuse: { agent: "acuse", status: status("acuse") } }, agentConclusions, ticket, similarReports: [], similarCount: record.similarCount ?? 0, followUps: [], notes: [], auditLog: [] };
}
